...
class Optimization:
    ...
    def perform_optimization(
        ...
        v2g_soc_init: Optional[float] = None,
        ...
        if self.costfun == "power_peak_tariffs":
            if self.power_tariff_company == "ellevio":
                ...
                set_I_day = set_I[tariff_window_start:tariff_window_end]
                set_I_night = np.concatenate((set_I[:tariff_window_start], set_I[tariff_window_end:]))
                tariff_cost = 81.25         # Kr/kW
                transfer_cost = 0.0625      # Kr/kWh
                fixed_cost = 365            # Kr/månad (med 16A säkring)
            if self.power_tariff_company == "goteborg_energi":
                tariff_cost = 45            # Kr/kW
                transfer_cost = 0.25        # Kr/kWh 
                fixed_cost = 185            # Kr/månad   
        ...
        if self.costfun == "power_peak_tariffs":
            if self.power_tariff_company == "ellevio":
                P_peak_day = plp.LpVariable(
                        cat="Continuous",
                        lowBound=0,
                        name="P_peak_day"
                    )

                P_peak_night = plp.LpVariable(
                        cat="Continuous",
                        lowBound=0,
                        name="P_peak_night"
                    )

                peak_is_day = plp.LpVariable(
                    cat="Binary", name="peak_is_day"
                )
        ...
        v2g_SOC_final = plp.LpVariable(cat="Continuous", lowBound=0, upBound= 1, name="v2g_SOC_final")
        ...
        if self.optim_conf["set_use_v2g"]:
            v2g_P_sto_pos = {
                (i): plp.LpVariable(
                    cat="Continuous",
                    lowBound=0,
                    upBound=self.plant_conf["v2g_battery_discharge_power_max"],
                    name="v2g_P_sto_pos_{0}".format(i),
                )
                for i in set_I
            }
            v2g_P_sto_neg = {
                (i): plp.LpVariable(
                    cat="Continuous",
                    lowBound=-self.plant_conf["v2g_battery_charge_power_max"],
                    upBound=0,
                    name="v2g_P_sto_neg_{0}".format(i),
                )
                for i in set_I
            }
        else:
            v2g_P_sto_pos = {(i): i * 0 for i in set_I}
            v2g_P_sto_neg = {(i): i * 0 for i in set_I}
        ...
        if self.costfun != "power_peak_tariffs":
            ...
            # Cost of having a low battery at the end of the optimization window
            objective -= 0.001 * (self.plant_conf["v2g_battery_maximum_state_of_charge"] - v2g_SOC_final) * self.plant_conf["v2g_battery_nominal_energy_capacity"] * unit_load_cost[0]
        ...
        # Add more terms to the objective function in the case of v2g battery use
        if self.optim_conf["set_use_v2g"]:
            objective = objective + plp.lpSum(
                -0.001
                * self.timeStep
                * (
                    self.optim_conf["v2g_weight_battery_discharge"] * v2g_P_sto_pos[i]
                    - self.optim_conf["v2g_weight_battery_charge"] * v2g_P_sto_neg[i]
                )
                for i in set_I
            )
        ...
        if self.optim_conf["set_use_v2g"]:
            ...
            v2g_SOC_opt_penalty = np.zeros(n)
                ...
                constraints.update(
                    {
                        "v2g_constraint_usesocmin_{}".format(k): plp.LpConstraint(
                            e=plp.lpSum(
                                ...
                                * v2g_P_sto_neg[i]
                                + v2g_SOC_opt_penalty[i] * self.plant_conf["v2g_battery_nominal_energy_capacity"]
                                / self.timeStep 
                ...
                if win_start > 0:
                    for j in range (win_end_prev, win_start):
                        if window_start_timestep[0] != 0:
                            v2g_SOC_opt_penalty[j] = vehicle_leave_subtract[k]/(win_start - win_end_prev)
                        elif len(window_start_timestep) > 0:
                            v2g_SOC_opt_penalty[j] = vehicle_leave_subtract[k-1]/(win_start - win_end_prev)
                    if k-1 == -1:
                        constraints.update(
                        {
            ...
            if self.optim_conf["v2g_set_nocharge_from_grid"]:
                constraints.update(
                    {
                        "v2g_constraint_nocharge_from_grid_{}".format(i): plp.LpConstraint(
                            e=v2g_P_sto_neg[i] + P_PV[i], sense=plp.LpConstraintGE, rhs=0
                        )
                        for i in set_I
                    }
                )
            # Optional constraints to avoid discharging the battery to the grid
            if self.optim_conf["v2g_set_nodischarge_to_grid"]:
                constraints.update(
                    {
                        "v2g_constraint_nodischarge_to_grid_{}".format(i): plp.LpConstraint(
            ...
            if self.optim_conf["v2g_set_battery_dynamic"]:
                constraints.update(
                    {
                        "v2g_constraint_pos_batt_dynamic_max_{}".format(
                            i
                        ): plp.LpConstraint(
                            e=v2g_P_sto_pos[i + 1] - v2g_P_sto_pos[i],
                            sense=plp.LpConstraintLE,
                            rhs=self.timeStep
                            * self.optim_conf["v2g_battery_dynamic_max"]
                            * self.plant_conf["v2g_battery_discharge_power_max"],
                        )
                        for i in range(n - 1)
                    }
                )
                constraints.update(
                    {
                        "v2g_constraint_pos_batt_dynamic_min_{}".format(
                            i
                        ): plp.LpConstraint(
                            e=v2g_P_sto_pos[i + 1] - v2g_P_sto_pos[i],
                            sense=plp.LpConstraintGE,
                            rhs=self.timeStep
                            * self.optim_conf["v2g_battery_dynamic_min"]
                            * self.plant_conf["v2g_battery_discharge_power_max"],
                        )
                        for i in range(n - 1)
                    }
                ...
                constraints.update(
                    {
                        "v2g_constraint_neg_batt_dynamic_max_{}".format(
                            i
                        ): plp.LpConstraint(
                            e=v2g_P_sto_neg[i + 1] - v2g_P_sto_neg[i],
                            sense=plp.LpConstraintLE,
                            rhs=self.timeStep
                            * self.optim_conf["v2g_battery_dynamic_max"]
                            * self.plant_conf["v2g_battery_charge_power_max"],
                        )
                        for i in range(n - 1)
                    }
                )
                constraints.update(
                    {
                        "v2g_constraint_neg_batt_dynamic_min_{}".format(
                            i
                        ): plp.LpConstraint(
                            e=v2g_P_sto_neg[i + 1] - v2g_P_sto_neg[i],
                            sense=plp.LpConstraintGE,
            ...
            constraints.update(
                {
                    "v2g_constraint_pstopos_{}".format(i): plp.LpConstraint(
                        e=v2g_P_sto_pos[i]
                        - self.plant_conf["v2g_battery_discharge_efficiency"]
                        * self.plant_conf["v2g_battery_discharge_power_max"]
                        * E[i],
                        sense=plp.LpConstraintLE,
                        rhs=0,
            ...
            constraints.update(
                {
                    "v2g_constraint_pstoneg_{}".format(i): plp.LpConstraint(
                        e=-v2g_P_sto_neg[i]
                        - (1 / self.plant_conf["v2g_battery_charge_efficiency"])
                        * self.plant_conf["v2g_battery_charge_power_max"]
                        * (1 - E[i]),
                        sense=plp.LpConstraintLE,
                        rhs=0,
            ...
            constraints.update(
                {
                    "v2g_constraint_socmax_{}".format(i): plp.LpConstraint(
                        e=-plp.lpSum(
                            v2g_P_sto_pos[j]
                            * (1 / self.plant_conf["v2g_battery_discharge_efficiency"])
                            + self.plant_conf["v2g_battery_charge_efficiency"]
                            * v2g_P_sto_neg[j]
                            + v2g_SOC_opt_penalty[j] * self.plant_conf["v2g_battery_nominal_energy_capacity"]
                                / self.timeStep
                            for j in range(i)
                        ),
                        sense=plp.LpConstraintLE,
            ...
            constraints.update(
                {
                    "v2g_constraint_socmin_{}".format(i): plp.LpConstraint(
                        e=plp.lpSum(
                            v2g_P_sto_pos[j]
                            * (1 / self.plant_conf["v2g_battery_discharge_efficiency"])
                            + self.plant_conf["v2g_battery_charge_efficiency"]
        ...
        # Power peak tariff contraints
        if self.costfun == "power_peak_tariffs":
            if self.power_tariff_company == "ellevio":
                constraints.update(
                    {
                        "constraint_peak_is_day": plp.LpConstraint(
                            e=M*peak_is_day - P_peak_day, sense=plp.LpConstraintGE, rhs=0
                        )
                    }
                )
                constraints.update(
                    {
                        "constraint_peak_is_night": plp.LpConstraint(
                            e=M*(1-peak_is_day) - P_peak_night, sense=plp.LpConstraintGE, rhs=0
                        )
                    }
                )
                constraints.update(
                    {
                        "constraint_power_peak_tariffs_day{}".format(i): plp.LpConstraint(
                            e=P_grid_pos[i] - (P_peak_day + 0.5*P_peak_night), sense=plp.LpConstraintLE, rhs=0
                        )
                        for i in set_I_day
                    }
                )
                constraints.update(
                    {
                        "constraint_power_peak_tariffs_night{}".format(i): plp.LpConstraint(
                            e=P_grid_pos[i] - (2*P_peak_day + P_peak_night), sense=plp.LpConstraintLE, rhs=0
                        )
                        for i in set_I_night
                    }
                )
            elif self.power_tariff_company == "goteborg_energi":
                constraints.update(
        ...
        # Slack constraint
        if self.costfun != "power_peak_tariffs" and self.optim_conf["set_use_tariff_peaks"]:
            constraints.update({
        ...
        if self.optim_conf["set_use_v2g"]:
            opt_tp["P_v2g"] = [
                v2g_P_sto_pos[i].varValue + v2g_P_sto_neg[i].varValue for i in set_I
            ]
            v2g_SOC_opt_delta = [
                (
                    v2g_P_sto_pos[i].varValue
                    * (1 / self.plant_conf["v2g_battery_discharge_efficiency"])
                    + self.plant_conf["v2g_battery_charge_efficiency"]
                    * v2g_P_sto_neg[i].varValue
                )
                * (self.timeStep / (self.plant_conf["v2g_battery_nominal_energy_capacity"]))
                for i in set_I
            ]
            v2g_SOC_opt_delta += v2g_SOC_opt_penalty

            v2g_SOCinit = copy.copy(v2g_soc_init)
            v2g_SOC_opt = []
            for i in set_I:
                v2g_SOC_opt.append(v2g_SOCinit - v2g_SOC_opt_delta[i])
                v2g_SOCinit = v2g_SOC_opt[i]
            opt_tp["v2g_SOC_opt"] = v2g_SOC_opt
        if self.plant_conf["inverter_is_hybrid"]:
        ...
        if self.costfun == "profit":
            ...
            opt_tp["cost_fun_ppt"] = [
                    ...
                    * (
                        unit_prod_price[i] * P_grid_neg[i].varValue
                        + transfer_cost * P_grid_pos[i].varValue
                    )
                    - fixed_cost / (len(set_I) * days_this_month)
                    for i in set_I
            ]
            # Calculates the power peak position and subtracts the cost of the peak there
            if self.power_tariff_company == "ellevio":
                P_grid_pos_values = {i: plp.value(P_grid_pos[i]) for i in P_grid_pos}
                if P_peak_night.varValue != 0:
                    max_index = max(P_grid_pos_values, key=P_grid_pos_values.get)
                else:
                    P_grid_pos_items = list(P_grid_pos_values.items())
                    P_grid_pos_values_day = dict(P_grid_pos_items[tariff_window_start:tariff_window_end])
                    max_index = max(P_grid_pos_values_day, key=P_grid_pos_values_day.get)
                opt_tp["cost_fun_ppt"][max_index] -= 0.001 * (P_peak_day.varValue + 0.5 * P_peak_night.varValue) * tariff_cost / days_this_month
            else:
                P_grid_pos_values = {i: plp.value(P_grid_pos[i]) for i in P_grid_pos}
                max_index = max(P_grid_pos_values, key=P_grid_pos_values.get)
                opt_tp["cost_fun_ppt"][max_index] -= 0.001 * P_peak.varValue * tariff_cost / days_this_month         
        else:
            self.logger.error("The cost function specified type is not valid")
    ...
    def perform_naive_mpc_optim(
        ...
        soc_final: Optional[float] = None,
        v2g_soc_init: Optional[float] = None,
        v2g_soc_final: Optional[float] = None,
        def_total_hours: Optional[list] = None,
        def_total_timestep: Optional[list] = None,
        def_start_timestep: Optional[list] = None,
        def_end_timestep: Optional[list] = None,
